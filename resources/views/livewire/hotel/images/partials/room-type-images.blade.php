<x-admin.card.base>
    {{-- Header with Room Type Selector and Upload Button --}}
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100">Room Type Images</h3>
            <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">
                Upload images for room types - they will appear in the gallery below
            </p>
        </div>

        <div class="flex items-center gap-3">
            {{-- Room Type Selector --}}
            <select wire:model.live="selectedRoomType" wire:change="changeRoomType($event.target.value)"
            ...
            <x-admin.button.primary
                wire:click="openRoomTypeUploadModal"
    ...
    {{-- Gallery Section (shows all room types) --}}
    @if(!empty($allRoomTypeImages))
        <div class="space-y-8">
            @foreach($roomTypes as $type)
                @if(isset($allRoomTypeImages[$type]) && count($allRoomTypeImages[$type]) > 0)
                    {{-- Room Type Header/Divider --}}
                    <div>
                        <div class="flex items-center mb-4">
                            <h4 class="text-base font-semibold text-gray-900 dark:text-gray-100">{{ $type }}</h4>
                            <div class="flex-1 ml-4 border-t border-gray-300 dark:border-gray-600"></div>
                        </div>

                        {{-- Images Grid --}}
                        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                            @foreach($allRoomTypeImages[$type] as $image)
                                <x-admin.card.base
                                    wire:key="room-type-{{ $image->id }}"
                                    padding="p-0"
                                    ...
                                    <div class="absolute top-2 right-2 z-10 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                        @if(!$loop->first)
                                            <button wire:click="moveImageUp({{ $image->id }}, 'room_type')"
                                        ...
                                        @endif
                                        @if(!$loop->last)
                                            <button wire:click="moveImageDown({{ $image->id }}, 'room_type')"
                                                class="bg-white dark:bg-gray-800 rounded p-1 shadow-md hover:bg-gray-100 dark:hover:bg-gray-700">
                                                <svg class="w-4 h-4 text-gray-600 dark:text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        <div class="flex items-center justify-between gap-2">
                                            @if(!$image->is_primary)
                                                <x-admin.button.secondary
                                                    wire:click="setPrimaryImage({{ $image->id }}, 'room_type')"
                                                    size="sm"
                                                    class="flex-1">
                                                    Set Primary
                                                </x-admin.button.secondary>
                                            ...
                                            @endif
                                            <button
                                                wire:click="confirmDeleteImage({{ $image->id }}, 'room_type')"
    ...
    @else
        <x-admin.card.empty-state
            title="No room type images yet"
            description="Upload images using the controls above. Images will be grouped by room type in the gallery."
            ...
            <x-slot:action>
                <x-admin.button.primary wire:click="openRoomTypeUploadModal">
                    Upload First Images
                </x-admin.button.primary>
            </x-slot:action>
        </x-admin.card.empty-state>
